...
public class isReachable extends AsyncTask<Void, Integer, Wrapper> {
    ...
    isReachable(int numPing, int waitPing, String ipping, TextView pingResults, TextView pingStats, String serverName, Context context){
        ...
        try {
            ...
            ArrayList<Integer> arrayPings = new ArrayList<>();
                ...
                for (int i = 0; i < splitString.length; i++) {
                    if (!splitString[i].contains("requests") && !splitString[i].contains("timed") && !splitString[i].contains("out")) {

                        int pingMilli = Integer.parseInt(splitString[i]);
